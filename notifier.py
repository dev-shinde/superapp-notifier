...
def main():
    ...
    if plan:
        ...
        # No plan rows yet — not done, needs onboarding nudge
        all_done = False
    ...
    if kb_empty or not plan:
        # App not used yet — only nudge during waking hours (8AM - 10PM)
        if now.hour < 8 or now.hour >= 22:
            print(f"  KB empty but outside waking hours ({now.hour}h) — skipping")
            return
        body = (
            f"👋 Good morning, {NAME}!\n\n"
